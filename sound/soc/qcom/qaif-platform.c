						       ...
						       size_t alloc_size)
{
	...
	dma_mem_info = kzalloc_obj(struct qaif_dma_mem_info, GFP_KERNEL);
	if (!dma_mem_info) {
		dev_err(component->dev, "error allocating dma_mem\n");
		return NULL;
	}
	dma_mem_info->alloc_size = alloc_size;
	dma_mem_info->vaddr = dma_alloc_wc(component->dev, alloc_size,
					   &dma_mem_info->dma_addr, GFP_KERNEL);
	if (!dma_mem_info->vaddr) {
		dev_err(component->dev, "error allocating DMA buffer\n");
		goto free;
	}
				    ...
				    struct qaif_dma_mem_info *dma_info)
{
	...
	dma_free_wc(dev, dma_info->alloc_size, dma_info->vaddr,
		    dma_info->dma_addr);
	kfree(dma_info);
}
				     ...
				     struct snd_pcm_substream *substream)
{
	...
	case MI2S_SEPTENARY:
		...
		buf->area = drvdata->aif_dma_heap[stream_dma_idx]->vaddr;
	...
	case LPASS_CDC_DMA_VA_TX0 ... LPASS_CDC_DMA_VA_TX8:
		...
		buf->area = drvdata->cif_dma_heap[stream_dma_idx]->vaddr;
